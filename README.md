...
It works offline. No account, no telemetry, nothing running in the background.
...
- Physical and virtual memory, plus the applications using the most of it.
...
are drawn in `OnRender`, themes and translations are resource dictionaries swapped at runtime. The
app displays about twenty numbers that change every second, so the extra layers would not earn
their keep.
...
Thermalyn/
  ...
  Controls/          history chart, component card, ring gauge, colour wheel
...
`SensorSelector` matches sensors by exact name. No nearest match, no "take the hottest one". If it
doesn't recognise a sensor it returns nothing and the UI shows a dash, because a wrong number that
looks plausible is worse than no number. [`docs/sensors.md`](docs/sensors.md) has the details,
including the sensor that reports processor load under a name starting with GPU.
...
Thermalyn is maintained by one person: [@NoaSansH](https://github.com/NoaSansH), who writes,
reviews and approves every change and publishes every release. Releases are built by GitHub Actions
from a tag, never from a developer machine, and each binary carries a signed provenance attestation.
