...
public sealed class HardwareMonitorService : IDisposable
{
    ...
    // MemoryGroup probes SPD synchronously and can take tens of seconds. Opened last, off the UI path.
    ...
    // WMI inventory is slow and static while the app runs.
    ...
    // SMART is slow to read and slow to change. Read it every tenth pass; sensors keep their
    // last values in between.
    ...
    private void OpenNextStage()
    {
        ...
        switch (_stage)
        {
            ...
            case 1:
                ...
                return;
            case 2:
                // MemoryGroup builds its inventory before taking the Computer.Add() lock.
    ...
    /// <summary>Timing log, enabled by creating an empty diagnostic.log in the settings folder.</summary>
    ...
    public HardwareSnapshot Read()
    {
        try
        {
            ...
            var memoryHardware = all.FirstOrDefault(h => h.HardwareType == HardwareType.Memory);
        ...
        {
            // One group per read, so no single read pays for all of them
            OpenNextStage();
        }
    }
    ...
    {
        // A busy discrete card beats the iGPU. Asleep, it scores nothing and the iGPU wins.
    ...
    // Intel Arc reports as GpuIntel, and "Radeon RX" is not "Radeon Graphics".
    ...
    // MSFT_PhysicalDisk media types: 3 = HDD, 4 = SSD, 5 = storage-class memory. Anything else stays generic.
    ...
    private static void EnrichMemory(ComponentReading memory)
    {
        try
        {
            ...
            memory.Load ??= total > 0 ? used / total * 100 : null;
            ...
            memory.Details.Add(new SensorReading(LocalizationService.Get("Hardware.VirtualTotal"), "Data", status.TotalPageFile / 1073741824d, "GB"));
            memory.Details.Add(new SensorReading(LocalizationService.Get("Hardware.VirtualAvailable"), "Data", status.AvailablePageFile / 1073741824d, "GB"));
        }
        catch { }
    }
