...
// Threshold crossings, one entry per episode, with the peak reached.
public sealed class ThermalEventService
{
    ...
    /// <summary>Persisted level tokens, never displayed: see Common.Hot and Common.Critical.</summary>
    ...
    public void Save()
    {
        ...
        try
        {
            ...
            File.WriteAllText(FilePath, JsonSerializer.Serialize(_entries, WriteOptions));
            _dirty = false;
        }
        catch { }
    }
    ...
    public bool Track(string component, double? temperature, int hotThreshold, int criticalThreshold)
    {
        ...
        if (_open.TryGetValue(component, out var current))
        {
            ...
            // Once critical, the whole episode counts as critical.
