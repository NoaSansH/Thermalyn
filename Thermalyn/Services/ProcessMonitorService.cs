...
public sealed partial class ProcessMonitorService
{
    ...
    private Dictionary<int, double> ReadGpuUsage(DateTime now)
    {
        ...
        if (_gpuCountersMissing) return result;
        try
        {
            // Hundreds of instances: one ReadCategory, not one counter each. Raw values are
            // busy time in 100 ns units.
        ...
        {
            // Absent without a WDDM 2.0 driver, and it will not appear before a restart.
            _gpuEngine = null;
