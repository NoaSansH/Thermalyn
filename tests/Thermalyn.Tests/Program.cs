...
var amdCpu = new[]
{
    ...
    S("Core (Tctl/Tdie)", SensorType.Temperature, 90), S("CCD1 (Tdie)", SensorType.Temperature, 90),
...
// An APU exposes GPU Core and GPU SoC power, but the GPU Core rail follows processor load.
// No graphics power must be reported from it.
var amdApuGpu = new[]
{
...
// A discrete Radeon does publish a board total.
...
// "GPU Package" exists only when NVML initializes. The Load rails are percentages, not watts.
...
Console.WriteLine("Sensor selection: 23 checks passed.");
...
if (args.Contains("--repository-contracts", StringComparer.OrdinalIgnoreCase))
{
    ...
    // The vocabulary is taken from the French table minus everything the English one also uses, so
    // it follows the translations instead of being maintained by hand. Homographs shared by both
    // languages are listed once here.
    string[] homographs =
    [
        ...
        "diagnostic",
        // Palette identifiers persisted in the settings. The labels beside them are localized.
        "ardoise", "prisme", "menthe", "cuivre", "sauge", "sable", "encre", "grenat", "studio"
    ];
    ...
    foreach (var file in Directory.EnumerateFiles(repository, "*", SearchOption.AllDirectories))
    {
        ...
        // A publish rewrites the lock file with a runtime-specific graph. Committing that breaks
        // the locked restore with an unhelpful NU1004.
        ...
        // User-visible strings belong in the two string tables. Words without accents slip past a
        // visual review, so they are checked by name. Application only: the tables and this file
        // legitimately contain French.
    ...
    Console.WriteLine($"Repository contracts: {english.Count} bilingual strings, UI, installer, manifest, privacy and language checks passed.");
}
