...
The manifests describe a release that is already published, because their checksums are taken
from its `SHA256SUMS.txt`. They are updated after a release, not with it, and so trail the
repository version by one.
...
choco pack
choco push thermalyn.1.0.2.nupkg --source https://push.chocolatey.org/ --api-key <key>
```
