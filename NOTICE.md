...
The repository does not contain the driver, and neither does the application: it points at the
official download page when a reading needs it. `tools/fetch-pawnio.ps1` downloads a pinned version
at build time and verifies its SHA-256, so the installers always bundle a known, unmodified copy.

`Thermalyn-Setup-Offline.exe` is the one exception: it bundles the unmodified PawnIO installer so
that a machine without an internet connection can be set up in one pass. That release asset is a
redistribution of a GPLv2 work; its complete corresponding source is the upstream repository linked
above, and the author offers it there. `Thermalyn-Setup.exe` bundles nothing and installs the
driver from its official source instead.
